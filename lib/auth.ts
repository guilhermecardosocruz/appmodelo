 ...
 *
 * Regra:
 * - Se existir usuário com appleId -> usa ele.
 * - Senão, se existir com mesmo e-mail -> vincula appleId nesse usuário.
 * - Senão, cria um novo usuário com senha aleatória (não usada pelo app).
 */
export async function findOrCreateAppleUser(params: {
  appleId: string;
  email: string;
  name?: string | null;
}) {
  ...
  if (byEmail) {
    // Se já existe por e-mail mas sem appleId, vincula
    if (!byEmail.appleId) {
      const updated = await prisma.user.update({
        where: { id: byEmail.id },
        data: { appleId: params.appleId },
      });
    ...
    // Já tinha algum appleId (caso raro, mas mantemos)
    return {
      id: byEmail.id,
      name: byEmail.name,
      email: byEmail.email,
