...
export default function DashboardClient() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-50">
      {/* espaço para o AppTopbar fixo */}
      <div className="h-14" />

      <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <header className="flex items-center justify-between gap-3">
          <h1 className="text-lg font-semibold sm:text-xl">Meus eventos</h1>
        ...
        <form
          ...
          className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-end sm:gap-4 dark:border-slate-800 dark:bg-slate-900/60 dark:shadow-none"
        >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {events.map((event) => (
            <div
              ...
              className="cursor-pointer rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-emerald-500/70 dark:border-slate-800 dark:bg-slate-900/60 dark:shadow-none dark:hover:bg-slate-800/60"
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-lg border border-red-600 px-2.5 py-1 text-[11px] font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/30"
                >
                  Excluir
                </button>
              </div>

              <h2 className="mt-2 line-clamp-2 text-sm font-semibold text-slate-900 dark:text-slate-50">
                {event.name}
              </h2>
            </div>
          ))}
