  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between gap-3">
        <h1 className="text-lg sm:text-xl font-semibold text-slate-50">Meus eventos</h1>
      </div>

      {/* Form SEMPRE escuro (não depende de dark:) */}
      <form
        onSubmit={handleCreate}
        className="flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-900/40 p-4 shadow-sm sm:flex-row sm:items-end sm:gap-4"
      >
      ...
      {loading && <p className="mt-4 text-sm text-slate-300">Carregando eventos...</p>}
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
        {events.map((event) => (
          <div
            ...
            className="w-full min-w-0 h-full flex cursor-pointer flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-900/40 p-4 shadow-sm transition hover:border-emerald-500/70 hover:bg-slate-900/60"
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                className="shrink-0 inline-flex items-center justify-center rounded-lg border border-red-500/80 px-2.5 py-1 text-[11px] font-medium text-red-300 hover:bg-red-950/40"
              >
                Excluir
              </button>
            </div>

            <h2 className="min-w-0 text-sm font-semibold text-slate-50 line-clamp-2">
              {event.name}
            </h2>
          </div>
        ))}
